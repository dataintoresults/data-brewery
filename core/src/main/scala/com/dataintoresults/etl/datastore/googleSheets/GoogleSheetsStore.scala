...
class GoogleSheetsStore extends EtlDatastore with DataStore {
  ...
  def createDataSink(gsTable: GoogleSheetsTable) : DataSink = {
    ...
    new DataSink {      
      ...
      private var _structure = gsTable.columns
      ...
      def close() = {    
        val spreadsheet = sheets.spreadsheets().get(gsTable.spreadsheetId).execute()
        val sheet = spreadsheet.getSheets.find(_.getProperties.getSheetId == sheetId).get

        // Clean unneeded data
        logger.debug("Google Sheet: Fill the rest of the page with empty cells")
        val lastRowNum = sheet.getProperties.getGridProperties.getRowCount
        val lastFilledRowNum = rowIndex + rows.length
        // +1 as hasNext will increment automatically by 1
        (lastFilledRowNum+1) until lastRowNum foreach { i =>
            rows.add(
              new RowData().setValues(
                structure.map(c => new CellData())
              )
            )
        }
